...
//studentClass.setText(studentClassStr);
        ...
        enterMessage.setOnTouchListener(OnTouchListener { v, event ->
        ...
        submitBtn.setOnClickListener(View.OnClickListener {
            ...
            //else
            if (enterMessage.getText().toString().trim { it <= ' ' } == "") {
                AppUtils().showDialogAlertDismiss(
                    mContext as Activity?,
    ...
    private fun submitLeave() {
        val Date: String = AppUtils().dateConversionY(AppUtils().getCurrentDateToday())

        val call: Call<CommonResponseModel> = ApiClient.getClient.requestleave( PreferenceManager().getaccesstoken(mContext).toString(),
            studentIdStr,    PreferenceManager().getUserId(mContext).toString(),Date,Date,
        ...
        call.enqueue(object : Callback<CommonResponseModel> {
            ...
            override fun onResponse(
                ...
                if (response.body()!!.responsecode.equals("200")) {
                    ...
                    } else if (response.body()!!.response.statuscode == "211") {
                        AppUtils().showDialogAlertDismiss(
                            mContext as Activity,
                    ...
                    } else if (response.body()!!.response.statuscode == "313") {
                        AppUtils().showDialogAlertDismiss(
                            mContext as Activity,
                            getString(R.string.error_heading),
                            "Already requested",
  ...
  /*  override fun onResume() {
        ...
        if (!getUserId(mContext).equalsIgnoreCase("")) {
            ...
            AppController.getInstance().trackScreenView(
                "Leave Request Submission." + "(" + PreferenceManager.getUserEmail(mContext) + ")" + " " + "(" + Calendar.getInstance().time + ")"
            )
        }
    } */
}